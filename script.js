...
function initRegistrationApp() {
    ...
    function updateSubmitBtn() {
        ...
        if (isFree || finalRs <= 0) {
            ...
            const trId = 'CB' + Date.now();
            const qrData = `upi://pay?pa=${upiId}&pn=ChessBird&tr=${trId}&am=${finalDisplay}&cu=INR`;
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            const handleFinalSave = async (paymentId, usedPromoCode, discountApplied, finalAmountPaise) => {
                ...
                if (isLocalhost) {
                    const fallbackUrl = window.activeVercelUrl || 'https://chessbird.online';
