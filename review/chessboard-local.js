...
;(function () {
  ...
  function constructor (containerElOrString, config) {
    ...
    function beginDraggingPiece (source, piece, x, y) {
      ...
      // calculate drag offset so piece doesn't jump to center
      if (source !== 'spare') {
        var sourceSquarePosition = $('#' + squareElsIds[source]).offset()
        if (sourceSquarePosition) {
          dragOffsetX = x - (sourceSquarePosition.left + squareSize / 2)
          dragOffsetY = y - (sourceSquarePosition.top + squareSize / 2)
        } else {
          dragOffsetX = 0
          dragOffsetY = 0
        }
      ...
      $draggedPiece.attr('src', buildPieceImgSrc(piece)).css({
        display: '',
        position: 'absolute',
        left: x - squareSize / 2 - dragOffsetX,
        top: y - squareSize / 2 - dragOffsetY
      })
    ...
    function updateDraggedPiece (x, y) {
      // put the dragged piece over the mouse cursor with offset
      $draggedPiece.css({
        left: x - squareSize / 2 - dragOffsetX,
        top: y - squareSize / 2 - dragOffsetY
      })
